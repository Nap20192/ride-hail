...
insert into
    "roles" ("value")
values ('PASSENGER'), -- Passenger/Customer
    ('DRIVER'), -- Driver
    ('ADMIN') -- Administrator
;
...
insert into
    "ride_status" ("value")
values ('REQUESTED'), -- Ride has been requested by customer
...
insert into
    "vehicle_type" ("value")
values ('ECONOMY'), -- Standard economy ride
...
create table coordinates (
    ...
    entity_type varchar(20) not null check (
        entity_type in ('driver', 'passenger')
    ),
...
create index idx_coordinates_current on coordinates (entity_id, entity_type)
where
    is_current = true;
...
values ('RIDE_REQUESTED'), -- Initial ride request
...
create table ride_events (
    ...
    event_type text references "ride_event_type" (value) not null,
...
*/
CREATE TABLE IF NOT EXISTS ride_counters (
